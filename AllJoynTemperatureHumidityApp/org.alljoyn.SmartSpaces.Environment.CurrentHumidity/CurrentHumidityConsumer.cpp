...
std::map<alljoyn_interfacedescription, WeakReference*> CurrentHumidityConsumer::SourceInterfaces;

CurrentHumidityConsumer::CurrentHumidityConsumer(AllJoynBusAttachment^ busAttachment)
...
CurrentHumidityConsumer::~CurrentHumidityConsumer()
{
...
void CurrentHumidityConsumer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void CurrentHumidityConsumer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void CurrentHumidityConsumer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
QStatus CurrentHumidityConsumer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
...
IAsyncOperation<CurrentHumidityJoinSessionResult^>^ CurrentHumidityConsumer::JoinSessionAsync(
    _In_ AllJoynServiceInfo^ serviceInfo, _Inout_ CurrentHumidityWatcher^ watcher)
{
    return create_async([serviceInfo, watcher]() -> CurrentHumidityJoinSessionResult^
    {
        auto result = ref new CurrentHumidityJoinSessionResult();
        result->Consumer = ref new CurrentHumidityConsumer(watcher->BusAttachment);
...
IAsyncOperation<CurrentHumidityGetVersionResult^>^ CurrentHumidityConsumer::GetVersionAsync()
{
    return create_async([this]() -> CurrentHumidityGetVersionResult^
    {
...
IAsyncOperation<CurrentHumidityGetCurrentValueResult^>^ CurrentHumidityConsumer::GetCurrentValueAsync()
{
    return create_async([this]() -> CurrentHumidityGetCurrentValueResult^
    {
...
IAsyncOperation<CurrentHumidityGetMaxValueResult^>^ CurrentHumidityConsumer::GetMaxValueAsync()
{
    return create_async([this]() -> CurrentHumidityGetMaxValueResult^
    {
        PropertyGetContext<byte> getContext;
        ...
        result->Status = getContext.GetStatus();
        result->MaxValue = getContext.GetValue();
        return result;
    });
}

void CurrentHumidityConsumer::OnPropertyChanged(_In_ alljoyn_proxybusobject obj, _In_ PCSTR interfaceName, _In_ const alljoyn_msgarg changed, _In_ const alljoyn_msgarg invalidated)
{
    UNREFERENCED_PARAMETER(obj);
    ...
    for (size_t i = 0; i < changedPropertyCount; i++)
    {
        ...
        {
            byte argument;
            (void)TypeConversionHelpers::GetAllJoynMessageArg(propertyValue, "y", &argument);
            CurrentValueChanged(this, (Platform::Object^)argument);
        }
        if (strcmp("MaxValue", propertyName) == 0)
        {
            byte argument;
            (void)TypeConversionHelpers::GetAllJoynMessageArg(propertyValue, "y", &argument);
            MaxValueChanged(this, (Platform::Object^)argument);
        }
    }
}

int32 CurrentHumidityConsumer::JoinSession(_In_ AllJoynServiceInfo^ serviceInfo)
{
    alljoyn_sessionlistener_callbacks callbacks =
    {
        AllJoynHelpers::SessionLostHandler<CurrentHumidityConsumer>,
        AllJoynHelpers::SessionMemberAddedHandler<CurrentHumidityConsumer>,
        AllJoynHelpers::SessionMemberRemovedHandler<CurrentHumidityConsumer>
    };
    ...
    PCSTR propertyNames[] = { "CurrentValue", "MaxValue" };
    ...
    RETURN_IF_QSTATUS_ERROR(alljoyn_proxybusobject_registerpropertieschangedlistener(
        ...
        AllJoynHelpers::PropertyChangedHandler<CurrentHumidityConsumer>, 
        m_weak));


    alljoyn_interfacedescription description = alljoyn_busattachment_getinterface(m_nativeBusAttachment, "org.alljoyn.SmartSpaces.Environment.CurrentHumidity");
