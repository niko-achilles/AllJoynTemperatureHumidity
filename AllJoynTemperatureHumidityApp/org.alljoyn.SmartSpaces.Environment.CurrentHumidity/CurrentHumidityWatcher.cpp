...
CurrentHumidityWatcher::CurrentHumidityWatcher(AllJoynBusAttachment^ busAttachment) :
...
CurrentHumidityWatcher::~CurrentHumidityWatcher()
{
    UnregisterFromBus();
}

void CurrentHumidityWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "org.alljoyn.SmartSpaces.Environment.CurrentHumidity" };
...
void CurrentHumidityWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "org.alljoyn.SmartSpaces.Environment.CurrentHumidity"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "org.alljoyn.SmartSpaces.Environment.CurrentHumidity"),
            port);
        Added(this, args);
    }
...
void CurrentHumidityWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<CurrentHumidityWatcher>
        };
...
void CurrentHumidityWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_CurrentHumidityIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &CurrentHumidityWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void CurrentHumidityWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void CurrentHumidityWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
