...
IAsyncOperation<CurrentTemperatureGetPrecisionResult^>^ CurrentTemperatureConsumer::GetPrecisionAsync()
{
    return create_async([this]() -> CurrentTemperatureGetPrecisionResult^
    {
        PropertyGetContext<double> getContext;
        ...
        result->Status = getContext.GetStatus();
        result->Precision = getContext.GetValue();
        return result;
    });
}

IAsyncOperation<CurrentTemperatureGetUpdateMinTimeResult^>^ CurrentTemperatureConsumer::GetUpdateMinTimeAsync()
{
    return create_async([this]() -> CurrentTemperatureGetUpdateMinTimeResult^
    {
        PropertyGetContext<uint16> getContext;
        ...
        result->Status = getContext.GetStatus();
        result->UpdateMinTime = getContext.GetValue();
        return result;
    });
}
...
void CurrentTemperatureConsumer::OnPropertyChanged(_In_ alljoyn_proxybusobject obj, _In_ PCSTR interfaceName, _In_ const alljoyn_msgarg changed, _In_ const alljoyn_msgarg invalidated)
{
    ...
    for (size_t i = 0; i < changedPropertyCount; i++)
    {
        ...
        if (strcmp("Precision", propertyName) == 0)
        {
            double argument;
            (void)TypeConversionHelpers::GetAllJoynMessageArg(propertyValue, "d", &argument);
            PrecisionChanged(this, (Platform::Object^)argument);
        }
        if (strcmp("UpdateMinTime", propertyName) == 0)
        {
            uint16 argument;
            (void)TypeConversionHelpers::GetAllJoynMessageArg(propertyValue, "q", &argument);
            UpdateMinTimeChanged(this, (Platform::Object^)argument);
        }
    }
}
...
int32 CurrentTemperatureConsumer::JoinSession(_In_ AllJoynServiceInfo^ serviceInfo)
{
    ...
    PCSTR propertyNames[] = { "CurrentValue", "Precision", "UpdateMinTime" };
    ...
    alljoyn_interfacedescription description = alljoyn_busattachment_getinterface(m_nativeBusAttachment, "org.alljoyn.SmartSpaces.Environment.CurrentTemperature");
